...
class DropControllerCollection {
  ...
  private final ArrayList<DropController> prioDropControllerList;
  ...
  DropControllerCollection(ArrayList<DropController> dropControllerList) {
    ...
    this.prioDropControllerList = null;
  }

  /**
   * Default constructor.
   */
  DropControllerCollection(ArrayList<DropController> dropControllerList, ArrayList<DropController> prioDropControllerList) {
    this.dropControllerList = dropControllerList;
    this.prioDropControllerList = prioDropControllerList;
  }
  ...
  DropController getIntersectDropController(int x, int y) {
    ...
    if ( prioDropControllerList != null ) {
    	for ( DropController prioDropController : prioDropControllerList ) {
    		WidgetArea targetArea = new WidgetArea(prioDropController.getDropTarget(), null);
			if (targetArea.intersects(location)) {
				return prioDropController;
			}
    	}
    }
    for (int i = sortedCandidates.length - 1; i >= 0; i--) {
